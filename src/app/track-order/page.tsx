...
export default function TrackOrderPage() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    try {
      // Lookup by BOTH ID and Phone for security
      ...
      if (fetchError || !data) {
        setError('Order not found. Please check your IDs and Phone Number and try again.');
      } else {
        setOrder(data);
      }
    } catch (err) {
      ...
      <main className="flex-1 container mx-auto px-6 py-16">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-accent text-[10px] font-black uppercase tracking-[0.3em] mb-4 inline-block">Order Lookup</span>
            <h1 className="text-5xl font-black tracking-tighter uppercase italic leading-none mb-6">Track Your Order</h1>
            <p className="text-gray-400 text-sm font-medium">Enter your Order ID and Phone Number to see real-time updates.</p>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-white border border-gray-100 rounded-2xl py-6 pl-16 pr-6 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all shadow-sm"
                    value={orderId}
            ...
            <div className="space-y-8 animate-slide-up">
              ...
              <div className="bg-white border border-gray-100 rounded-[2.5rem] p-10 shadow-sm relative overflow-hidden">
                ...
                <div className="relative h-1 bg-gray-100 rounded-full mb-12">
                    <div 
                     className={`absolute top-0 left-0 h-full transition-all duration-1000 ease-out rounded-full ${order.status === 'cancelled' ? 'bg-red-500 w-full' : 'bg-accent'}`}
                          ...
                          ['pending', 'processing', 'shipped', 'delivered'].map((step, i) => (
                            <div key={step} className="flex flex-col items-center">
                              ...
                              <span className={`text-[8px] font-black uppercase tracking-widest mt-4 ${getStatusStep(order.status) >= i ? 'text-black' : 'text-gray-300'}`}>
                                {t(`status_${step}` as keyof typeof translations['en'])}
                              </span>
                            </div>
                          ))
                       )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-gray-100 rounded-[2rem] p-8 shadow-sm">
                   ...
                   <p className="text-[11px] font-bold mb-1">Placed on {new Date(order.created_at).toLocaleDateString()}</p>
                   <p className="text-[11px] text-gray-400 font-medium">
                     {order.items?.length} Items • {order.total} MAD
                   </p>
                </div>
              </div>
            </div>
